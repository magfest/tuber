    ...
    <div>
        <tuber-table tableTitle="Events" formTitle="Event" url="/api/event" :filters="filters">
            ...
            <template #actions="tableProps">
                <Column header="Actions" style="width: 15rem">
                    <template #body="slotProps">
                        ...
                        <Button @click="tableProps.remove($event, slotProps.data).then(() => {reload_events()})" icon="pi pi-times"
            ...
            <template #formActions="props">
                ...
                <Button label="Save" @click="props.save(props.edited).then(() => {reload_events()})" icon="pi pi-check" autofocus />
            </template>
        </tuber-table>
    </div>
</template>
