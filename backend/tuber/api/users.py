...
def initial_setup():
    ...
    if request.json['username'] and request.json['email'] and request.json['password']:
        user = User(username=request.json['username'], email=request.json['email'],
                    password=sha256_crypt.hash(request.json['password']), active=True)
...
def login():
    if request.json['username'] and request.json['password']:
        user = db.query(User).filter(User.username ==
                                     request.json['username']).one_or_none()
        if user:
            if sha256_crypt.verify(request.json['password'], user.password):
                ...
                session = Session(user=user.id, badge=badge, last_active=datetime.datetime.now(
                ), secret=str(uuid.uuid4()), permissions=json.dumps(perm_cache))
                db.add(session)
                db.commit()
...
def logout():
    ...
    if 'session' in request.cookies:
        db.query(Session).filter(Session.secret ==
                                 request.cookies.get('session')).delete()
    db.commit()
    return "null", 200
...
@app.route("/api/user/me", methods=["GET", "PATCH"])
def get_current_user():
    if request.method == "GET":
        if g.user:
            return jsonify(User.serialize(g.user))
        return "", 404
    elif request.method == "PATCH":
        user = db.query(User).filter(User.id == g.user.id).one_or_none()
        if user:
            user.default_event = request.json['default_event']
            db.add(user)
            db.commit()
