...
def test_fast_request(client):
    ...
    result = client.get("/api/fast", handle_async=False)
    assert result.status_code == 200

def test_slow_request(client):
    """Make sure that slow requests do create a job."""
    result = client.get("/api/slow", handle_async=False)
...
def test_job_retrieval(client):
    ...
    while time.time() - start_time < 15:
        result = client.get(url, handle_async=False)
        time.sleep(0.2)
    ...
    assert "description" in result.json[0]
