...
for room_night in room_nights:
    if room_night['date'] in uber_room_nights.keys():
        room_nights_lookup[room_night['id']] = uber_room_nights[room_night['date']]
    #else:
...
hrr = get(f"{BASE_URL}/hotel_room_request")
...
for room in rooms:
    ...
    nights = [room_nights_lookup[x] for x in nights if x in room_nights_lookup]
    ...
    for ass in assign:
        ...
        if not badge in assigned:
            ...
            try:
                ...
                print(f"Failed to assign roommate {badges[badge]['uber_id']} ({badges[badge]['search_name']})")