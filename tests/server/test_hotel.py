...
def test_staffer_auth(mock_post, client):
    ...
    rv = client.post('/api/hotels/staffer_auth', data=json.dumps({"token": "123", "csrf_token": token}), content_type="application/json")
    assert(not json.loads(rv.data)['success'])

    rv = client.post('/api/hotels/staffer_auth', data=json.dumps({"token": "abc", "csrf_token": token}), content_type="application/json")
    assert(not json.loads(rv.data)['success'])
    clear_table("user")